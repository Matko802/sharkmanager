...
fn build_ui(app: &Application) {
    ...
    btn_up.set_tooltip_text(Some("Up (Alt+Up)"));
    let btn_home = Button::from_icon_name("go-home-symbolic");
    btn_home.set_tooltip_text(Some("Home"));

    let btn_hidden = ToggleButton::builder()
    ...
    btn_refresh.set_tooltip_text(Some("Refresh (F5)"));
    ...
    header.pack_start(&Separator::new(Orientation::Vertical));
    header.pack_start(&btn_hidden);
    header.pack_start(&btn_view_toggle);
    header.pack_start(&Separator::new(Orientation::Vertical));
    header.pack_start(&btn_new_folder);
    ...
    *active_pane.borrow_mut() = Some(first_pane.clone());
    ...
    }
    {
        ...
        btn_up.connect_clicked(move |_| {
            let Some(pane) = ap.borrow().as_ref().cloned() else { return };
            let cur = pane.state.current_path.borrow().clone();
            if let Some(parent) = cur.parent() {
                navigate_to(&pane.state, parent.to_path_buf());
                pane.refresh_fn()();
            }
        });
    }
    {
        ...
        btn_home.connect_clicked(move |_| {
            ...
            if let Some(home) = dirs::home_dir() {
                navigate_to(&pane.state, home);
                pane.refresh_fn()();
            }
        });
    }
    btn_hidden.connect_toggled({
        let ap = active_pane.clone();
        move |btn| {
            let Some(pane) = ap.borrow().as_ref().cloned() else { return };
            pane.state.show_hidden.set(btn.is_active());
            pane.refresh_fn()();
        }
    });
    ...
    btn_refresh.connect_clicked({
        let ap = active_pane.clone();
        move |_| {
            if let Some(pane) = ap.borrow().as_ref().cloned() {
                pane.refresh_fn()();
            }
        }
    });
    ...
    // Path entry handling (Ctrl+L)
    {
        let breadcrumb_box_ = breadcrumb_box.clone();
        let path_entry_ = path_entry.clone();
        let window_ = window.clone();
        let ap = active_pane.clone();
        // clicking on empty area? We'll add key controller to window for Ctrl+L
        let key_ctrl = gtk::EventControllerKey::new();
        let breadcrumb_box_c = breadcrumb_box_.clone();
        let path_entry_c = path_entry_.clone();
        let btn_hidden_k = btn_hidden.clone();
        let ap_k = ap.clone();
        key_ctrl.connect_key_pressed(move |_, key, _, mods| {
            ...
            if mods.contains(gdk::ModifierType::CONTROL_MASK) && key == gdk::Key::l {
                breadcrumb_box_c.set_visible(false);
                path_entry_c.set_visible(true);
            ...
            if mods.contains(gdk::ModifierType::CONTROL_MASK) && key == gdk::Key::h {
                btn_hidden_k.set_active(!pane.state.show_hidden.get());
                return glib::Propagation::Stop;
            }
            glib::Propagation::Proceed
        });
        ...
        path_entry.connect_activate({
            let breadcrumb_box = breadcrumb_box_.clone();
            let path_entry = path_entry_.clone();
            let ap = ap.clone();
            move |e| {
        ...
        {
            let breadcrumb_box_c = breadcrumb_box.clone();
            let path_entry_c = path_entry.clone();
            let key_ctl = gtk::EventControllerKey::new();
            key_ctl.connect_key_pressed(move |_, key, _, _| {
                if key == gdk::Key::Escape {
                    path_entry_c.set_visible(false);
                    breadcrumb_box_c.set_visible(true);
                    return glib::Propagation::Stop;
                }
                glib::Propagation::Proceed
            });
        ...
        // Click on header path? Add gesture to title_box to edit on click? Already breadcrumb buttons handle.
    }
        ...
        key.connect_key_pressed(move |_, key, _, mods| {
            ...
            if ctrl {
                match key {
                    ...
                    gdk::Key::n => {
                        // Ctrl+N handled? Use Ctrl+Shift+N for folder, Ctrl+N for file
                        if shift {
                            // already new folder button
                        } else {
...
fn sync_controls(shared: &Shared, pane: &Pane) {
    ...
    shared.btn_hidden.set_active(pane.state.show_hidden.get());
    shared
        .btn_view_toggle
...
fn build_pane(
    ...
    // --- status bar ---
    let statusbar = GtkBox::new(Orientation::Horizontal, 8);
    statusbar.add_css_class("shark-statusbar");
    ...
    statusbar.append(&status_sel);
    statusbar.append(&status_count);
    ...
    // --- refresh closure ---
    {
        ...
        let dr: RefreshFn = Rc::new(move || {
            ...
            // Breadcrumbs (if not editing)
            if !gtk::prelude::WidgetExt::is_visible(&path_entry) {
                rebuild_breadcrumbs(
                    &breadcrumb_box,
                    &cur,
                    {
            ...
            if entries.is_empty() {
                status_label.set_text(&format!("Empty — {}", cur.display()));
                status_count.set_text("0 items");
                status_sel.set_text("");
            } else {
                let dirs = entries.iter().filter(|e| e.is_dir).count();
                let files = entries.len() - dirs;
                status_label.set_text(&cur.display().to_string());
